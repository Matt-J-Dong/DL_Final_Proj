...
from models_md_a import JEPA_Model
...
def save_model(model, optimizer, epoch, batch_idx, learning_rate, dropout, lambda_cov, save_path="checkpoints_wandb"):
    ...
    if batch_idx == -1:
        save_file = os.path.join(
            save_path,
            f"jepa_model_a_epoch_{epoch}_final_lr_{learning_rate}_do_{dropout}_cov_{lambda_cov}.pth"
        )
    else:
        save_file = os.path.join(
            save_path,
            f"jepa_model_a_epoch_{epoch}_batch_{batch_idx}_lr_{learning_rate}_do_{dropout}_cov_{lambda_cov}.pth"
        )
...
def train_model(
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        # **New code**: Write losses to file
        with open("losses_a.txt", "a") as f:
