...
class Trainer:
    ...
    def save_model(self, model, epoch):
        ...
        save_file = os.path.join(save_path, f"jepa_model_new_{model_version_}epoch_{epoch}.pth")
    ...
    def train(self):
        ...
        model = JEPA_Model(device=self.device, repr_dim=model_size, action_dim=2, dropout_prob=0).to(self.device)
...
def main():
    ...
    wandb.init(
        project=f"DL_Final_Project_2024_new_model_{model_version}",
        config=config
    )
