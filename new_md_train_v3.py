...
from new_models_md_v3 import BYOL_Model  # Changed to BYOL_Model
...
path_data = "/scratch/mjd9571/DL_Final_Proj/data/DL24FA"

wandb.init(project=f"BYOL_Project_{model_version}")  # Start W&B run under a sweep agent
...
def save_model(model, optimizer, epoch, batch_idx, learning_rate, dropout, lambda_cov, probe_lr, save_path="checkpoints_wandb"):
    ...
    if batch_idx == -1:
        save_file = os.path.join(
            save_path,
            f"byol_model_epoch_{epoch}_final_lr_{learning_rate}_do_{dropout}_cov_{lambda_cov}_probe_{probe_lr}.pth"
        )
    else:
        save_file = os.path.join(
            save_path,
            f"byol_model_epoch_{epoch}_batch_{batch_idx}_lr_{learning_rate}_do_{dropout}_cov_{lambda_cov}_probe_{probe_lr}.pth"
        )
...
def load_latest_checkpoint(model, optimizer, learning_rate, dropout, lambda_cov, probe_lr, checkpoint_dir="checkpoints_wandb"):
    ...
    pattern = f"byol_model_epoch_*_lr_{learning_rate}_do_{dropout}_cov_{lambda_cov}_probe_{probe_lr}.pth"
...
def main():
    ...
    model = BYOL_Model(device=device, repr_dim=model_size, action_dim=2, dropout=dropout)
    model.to(device)
