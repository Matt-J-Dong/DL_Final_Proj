...
def load_data(device, batch_size=256, is_distributed=False, subset_size=1000):
...
def save_model(model, optimizer, epoch, batch_idx, learning_rate, dropout, lambda_cov, probe_lr, save_path="checkpoints_wandb"):
    ...
    if batch_idx == -1:
        save_file = os.path.join(
            save_path,
            f"jepa_model_n_energy_reg_epoch_{epoch}_final_lr_{learning_rate}_do_{dropout}_cov_{lambda_cov}_probe_{probe_lr}.pth"
        )
    else:
        save_file = os.path.join(
            save_path,
            f"jepa_model_n_energy_reg_epoch_{epoch}_batch_{batch_idx}_lr_{learning_rate}_do_{dropout}_cov_{lambda_cov}_probe_{probe_lr}.pth"
        )
...
def load_latest_checkpoint(model, optimizer, learning_rate, dropout, lambda_cov, probe_lr, checkpoint_dir="checkpoints_wandb"):
    ...
    pattern = f"jepa_model_n_energy_reg_epoch_*_lr_{learning_rate}_do_{dropout}_cov_{lambda_cov}_probe_{probe_lr}.pth"
...
def main():
    wandb.init(project="my_jepa_project_sweep_n_energy_reg_large", config={
        ...
        "epochs": 50,
        "target_average": 1.0
    })  # Updated project name to reflect version 'h_energy_reg_large'
    sweep_config = {
    "method": "grid",
    "parameters": {
        "momentum": {"values": [0.9, 0.99]},
        "batch_size": {"values": [256,512]},  # Increased batch sizes
        "probe_lr": {"values": [0.0005, 0.002, 0.008]},
        "lambda_cov": {"values": [0.4, 0.7]},
        "learning_rate": {"values": [5e-5, 1e-4, 5e-4]},
        "dropout": {"values": [0.3, 0.5]},  # Increased dropout rates
    ...
    dropout = wandb.config.get("dropout", 0.5)
    learning_rate = wandb.config.get("learning_rate", 1e-4)
    lambda_cov = wandb.config.get("lambda_cov", 0.7)
    momentum = wandb.config.get("momentum", 0.99)
    batch_size = wandb.config.get("batch_size", 256)
    ...
    model = JEPA_Model(device=device, repr_dim=512, action_dim=2, dropout=dropout)
    model.to(device)
