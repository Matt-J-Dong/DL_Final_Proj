...
class SimpleResNet(nn.Module):
    ...
    It does not assume the input image is strictly 64x64. We dynamically determine
    the fully connected layer's input dimension based on the actual input.
    """
    ...
    def __init__(self, repr_dim=256):
        ...
        self.conv2 = nn.Conv2d(128, 256, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, x):
        ...
        # Dynamically create fc if not created yet
        if self.fc is None:
            self.fc = nn.Linear(C * H * W, self.repr_dim).to(x.device)
...
class SimpleResNetModel(nn.Module):
    ...
    def forward(self, init_state, actions):
        B = init_state.size(0)
        T_minus_one = actions.size(1) if actions.ndim == 3 else 0
        T = T_minus_one + 1
