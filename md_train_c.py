...
import wandb
from configs import LRSchedule
...
def train_model(
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        with open("losses_c.txt", "a") as f:  # Changing file name to "losses_c.txt" to reflect model version change
...
def main():
    ...
    # Adding probe_lr options {0.002, 0.005, 0.010} in sweep config (not hardcoded here)
    # wandb.config will set probe_lr
        ...
        for lr in learning_rates:
            for cov in lambda_cov_values:
                wandb.init(project="my_jepa_project", config={
                    ...
                    "distance_function": "l2",
                    "lambda_cov": cov
                    # Add probe_lr in the sweep config to test {0.002, 0.005, 0.010}
                }, reinit=True)
                ...
                # Use the same probe_lr as above
                probe_lr = wandb.config.get("probe_lr", 0.0002)
