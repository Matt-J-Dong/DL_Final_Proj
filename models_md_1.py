...
class Encoder(nn.Module):
    def __init__(self, output_dim=64, dropout_prob=0.0):
        super(Encoder, self).__init__()
        # Replace the single kernel size of 7 with three back-to-back 3x3 kernels
        self.conv1 = nn.Sequential(
            nn.Conv2d(2, 8, kernel_size=3, stride=1, padding=1, bias=False),  # First 3x3 kernel
            nn.BatchNorm2d(8),
            nn.ReLU(inplace=True),
            nn.Conv2d(8, 16, kernel_size=3, stride=1, padding=1, bias=False),  # Second 3x3 kernel
            nn.BatchNorm2d(16),
            nn.ReLU(inplace=True),
            nn.Conv2d(16, 16, kernel_size=3, stride=2, padding=1, bias=False),  # Third 3x3 kernel with stride 2
            nn.BatchNorm2d(16),
            nn.ReLU(inplace=True)
        )
        ...
        # ResNet-like layers
        self.layer1 = self._make_layer(BasicBlock, in_channels=16, out_channels=16, num_blocks=2, stride=1)
        self.layer2 = self._make_layer(BasicBlock, in_channels=16, out_channels=32, num_blocks=2, stride=2)
        self.layer3 = self._make_layer(BasicBlock, in_channels=32, out_channels=64, num_blocks=2, stride=2)
